...
function pilihanKomputer() {
    ...
    if( comp > 0 && comp < 0.34 ^ comp == 0.5 ) { // bernilai TRUE >> (true, true, false) >> jika di isi comp < 0.3399 di setelah XOR maka akan bernilai true dan hasilnya akan false / kosong yg di tampilkan
        ...
        return "BATU";
    } else if( comp >= 0.68 && comp <= 1 ) { // bernilai TRUE >> (true, true) >> jika di isi comp == 1 setelah AND maka akan bernilai false semua dan hasilnya akan false / kosong yg di tampilkan
        return "KERTAS";
    } else {
        return "KOSONG";
    }
